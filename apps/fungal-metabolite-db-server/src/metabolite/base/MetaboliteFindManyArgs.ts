/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { MetaboliteWhereInput } from "./MetaboliteWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { MetaboliteOrderByInput } from "./MetaboliteOrderByInput";

@ArgsType()
class MetaboliteFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => MetaboliteWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => MetaboliteWhereInput, { nullable: true })
  @Type(() => MetaboliteWhereInput)
  where?: MetaboliteWhereInput;

  @ApiProperty({
    required: false,
    type: [MetaboliteOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [MetaboliteOrderByInput], { nullable: true })
  @Type(() => MetaboliteOrderByInput)
  orderBy?: Array<MetaboliteOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { MetaboliteFindManyArgs as MetaboliteFindManyArgs };
